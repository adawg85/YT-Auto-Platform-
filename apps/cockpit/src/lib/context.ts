...
/** process.env merged with decrypted account-page secrets (DB wins). */
export async function getMergedEnv(): Promise<Record<string, string | undefined>> {
  const db = getDb();
  return { ...process.env, ...(await loadSecretsEnv(db)) };
}
