...
| `artDirection` / `notes` / `thumbnailTemplate` | free text (each ≤ **6000 chars**) | steer the image models / pipeline; LLM-read standing guidance. (`thumbnailTemplate` raised from 800 → 6000 in ticket 01KY6F1X…; `musicMood` stays short at 800.) A validation error names the field + actual-vs-allowed length so a multi-field patch doesn't need bisecting. |
