...
**Nothing can predict a cap** — Content ID membership and per-track policies are not publicly queryable, and Buckley's own "Ongoing Copyright Issues" page names neither track. So the guard records OBSERVATION, not prediction: `audio_assets.shorts_blocked` + `shorts_blocked_note` (migration `0083`). The load-bearing piece is that the skip lives in **`pickChannelBedTrack`**, not just the attach paths — no operator chose Aphelion, the least-recently-used rotation cursor landed on it, which is exactly how the same block gets bought twice. On a short-format channel (`short` or `both`) the rotation now skips flagged tracks, `set_music_bed`/`set_production_music` refuse them, `list_audio_assets` reports the flag, and a merely-registered track attaches with a WARNING rather than a refusal (blanket-refusing registered tracks would empty the beds and push the pipeline onto a BILLED AI-generated bed). Long-form is deliberately untouched: there a claim monetises. If every bed track is flagged, the picker returns null and the caller falls back exactly as for an empty bed. 8 unit tests on the pure rule (`shorts-claim-risk.ts`); the DB-dependent picker was verified live against a real Postgres — with the flagged track at the head of the rotation by LRU it picks the next one, long-form still picks it, and an all-flagged bed returns null.
**Live data fixed alongside:** Aphelion flagged and removed from all four beds; the Shorts beds topped back up from the clean tracks (Dog-Eared Shorts 2→5, Wings Shorts 3→5, Lost Books Shorts 7→8) and Dog-Eared long refilled 1→8 weighted to the longest tracks; the blocked production re-pointed to Reverie so the operator's republish does not re-block. Also normalised 7 assets whose requiredCreditFormat used a hyphen where Buckley's canonical template uses an en dash — cosmetic, NOT the cause: his claims-release page states the Smart Content ID system matches on "Scott Buckley"/"@ScottBuckley"/"musicbyscottb", not the full string.
...
`promptSuffix` is authored that way) → `"Style: Style:"` on every authored shot of every distilled-style channel. **#96** `archivalStrength:"off"`
+ `visualMode:"ai_images"` were honoured — `archivalImagePolicy` is correct — but 23 of 102 shots were real photography because
`resume_production` **copied them from a grandparent** created before the setting changed (`styleSource`/`engineRequested`/`engineServed` all
null was the tell: never generated by that run). Now sourced stills are not carried into a production whose profile forbids sourcing, and #94's
carried per-video profile is **merged over the channel's current config** instead of freezing the ancestor's snapshot. **#97** the variation check
compared a production against **sibling productions of the same idea** — every recovery path mints siblings reusing the parent's
`substanceFingerprint`, so `jaccard=1.000` stranded $6.95 of human-approved work in `on_hold` right after the operator approved the visuals gate.
Corpus is now published/scheduled rows of OTHER ideas only, and `failureReason` names the counterparty. **#98** `force_forward` wrote status
`greenlit` — upstream of every completed stage including an approved gate — so a fully-built production read as "kicked back to scripting"; it now
presents the status matching the work that exists. Its sibling defect: `stuckReviewStates` watched only `*_review`, so a production stranded at
`greenlit` was invisible to the detector built to catch stranded productions → `stuckProductions()` watches every non-terminal status.
**#99 (SECURITY)** MCP receipts carried no client identity, so three unaccounted **billable** image generations were indistinguishable from a
leaked connector URL. Added attribution (migration `0071`): stable `clientId` from the handshake's `clientInfo` + a **salted hash** of the source
address (never the address), plus `targetChannelId`/`targetProductionId`; `get_diagnostics.mcpClients` roster; a billable call from a never-seen
client raises a **critical** alert pointing at `MCP_BEARER_TOKEN` rotation. 18 new tests (474 core); all typechecks + cockpit build.
**The roster fills from the next call — it cannot retroactively identify the 2026-08-03 calls.** All five left OPEN for the operator.
