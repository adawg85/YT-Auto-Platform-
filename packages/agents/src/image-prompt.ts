...
export async function buildImagePrompts(
  ...
  const system =
    ...
    "storefront, the same skyline — repeated across the whole video). Treat the " +
    "shots as a moving SEQUENCE: each frame must clearly differ from EVERY other " +
    "in composition, shot scale (wide/medium/close-up/insert) AND angle. When a " +
    "recurring motif (a diagram, a location, a device) must appear more than once, " +
    "render it from a NEW angle/scale/state each time — never the same picture " +
    "twice. When several sentences circle the SAME subject, deliberately change the " +
    "view — cut in to a detail, pull wide, switch to a reverse angle, feature a " +
    "different element, or advance the action a beat.\n" +
      ...
      ? `RECURRING CHARACTERS:\n${input.characters
          .map((c) => {
            const tag =
              c.castMode === "always"
                ? " (every scene)"
                : ["smart", "25", "50", "75"].includes(c.castMode ?? "")
                  ? " (recurring)"
                  : (c.role ?? "main") === "main"
                    ? " (main)"
                    : ` (${c.role})`;
